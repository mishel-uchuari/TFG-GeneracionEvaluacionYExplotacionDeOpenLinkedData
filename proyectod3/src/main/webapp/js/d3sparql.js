...
d3sparql.query = function(endpoint, sparql, callback) {
  var url = endpoint + encodeURIComponent(sparql)
  if (d3sparql.debug) { console.log(endpoint) }
  if (d3sparql.debug) { console.log(url) }
  var mime = "application/sparql-results+json"
  d3.xhr(url, mime).header("authorization", make_base_auth('admin', 'admin')).get(function(request) {
   var json = request.response
    if (d3sparql.debug) { console.log(json) }
    callback(JSON.parse(json))
  })

   /*d3.json(url).header("authorization", make_base_auth('admin', 'admin')).get(function(error, json) {
